...
const resolveWaveCount = (value, waveKey, enabledWaves) => {
  if (typeof value === 'number') return value;
  const idx = enabledWaves.indexOf(waveKey);
  return value[idx] ?? 6;
};

const resolveWaveDistance = (value, waveKey, enabledWaves) => {
  if (typeof value === 'number') return value;
  const idx = enabledWaves.indexOf(waveKey);
  return value[idx] ?? 5;
};
...
export function mountFloatingLines(container, options = {}) {
  ...
  const bottomLineDistance = enabledWaves.includes('bottom') ? resolveWaveDistance(lineDistance, 'bottom', enabledWaves) * 0.01 : 0.01;
...
export default function FloatingLines(props) {
  ...
  const mount = () => {
    ...
    const cleanup = mountFloatingLines(ref.current, props);
    ref.current.__floatingCleanup = cleanup;
    return cleanup;
  };

  const destroy = () => {
    if (!ref.current || !ref.current.__floatingCleanup) return;
    ref.current.__floatingCleanup();
    delete ref.current.__floatingCleanup;
  };

  if (typeof document !== 'undefined') {
    setTimeout(() => {
      if (ref.current) {
        if (ref.current.__floatingCleanup) destroy();
        mount();
      }
    }, 0);
  }
